...
from sdr_stream import (
    FRAME_BYTES, SAMPLE_RATE, SSB_MODES, close_device, default_audio_gain_for_mode, load_config,
)
import sdr_demod
...
try:
    while time.monotonic() < deadline:
        ...
        if result.ret > 0:
            ...
            default_gain = default_audio_gain_for_mode(config["mode"])
            audio_gain = float(os.environ.get("SDR_AUDIO_GAIN", str(default_gain)))
            received.extend(sdr_demod.audio_to_pcm16(audio, audio_gain))
